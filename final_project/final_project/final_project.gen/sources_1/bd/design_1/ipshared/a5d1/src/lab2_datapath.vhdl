...
architecture lab2_datapath_arch of lab2_datapath is
    ...
    Port ( clk : in STD_LOGIC;
        ...
        ac_lrclk : out STD_LOGIC;
        ready : out STD_LOGIC;
        L_bus_in : in std_logic_vector(17 downto 0);
        R_bus_in : in std_logic_vector(17 downto 0);
        L_bus_out : out  std_logic_vector(17 downto 0);
        R_bus_out : out  std_logic_vector(17 downto 0);
        scl : inout STD_LOGIC;
        sda : inout STD_LOGIC;
    ...
    signal cw_counter_control: std_logic;
    ...
    -- SFX wiring
    signal sfx_trig_q: std_logic := '0';
    signal sfx_start : std_logic;
    signal sfx_sample : std_logic_vector(15 downto 0);
...
begin
	...
	-- create a process for the sound player 
	process(clk)
    begin
        if rising_edge(clk) then
            if reset_n = '0' then
                ...
                sfx_trig_q <= sfx_trig_in;
            end if;
        end if;
    end process;
    
    -- wire up the player
    sfx_start <= sfx_trig_in and not sfx_trig_q;
    u_sound_player : entity work.sound_player
        port map (
            clk => clk,
            reset_n => reset_n,
            sample_tick => sw_ready,
            start => sfx_start,
            sound_id => sfx_id_in,
            busy => sfx_busy_sig,
            sample_out => sfx_sample
        );
	...
	audio_counter : entity work.counter
	  generic map(
        num_bits => 10,
        max_value => 1023
      )
    ...
    -- Instantiate the nes_controller
    nes_inst : entity work.nes_controller
    generic map (
        CLK_FREQ_HZ => 100000000,
        POLL_RATE_HZ => 60,
        BIT_HALF_PERIOD_US => 6
    )
    port map (
        clk => clk,
        reset_n => reset_n,
    ...
    tmds       => tmds,
    tmdsb      => tmdsb,
    barry_x    => unsigned(barry_x_in),
    barry_y    => unsigned(barry_y_in),
    zapper_x   => unsigned(zapper_x_in),
    zapper_y   => unsigned(zapper_y_in),
    missile_x  => unsigned(missile_x_in),
    missile_y  => unsigned(missile_y_in),
  ...
  Audio_Codec : Audio_Codec_Wrapper
    ...
    -- MODIFY THIS SO THAT I WRITE THE SFX SAMPLE DIRECTLY TO THE AUDIO CODEC FOR JETPACK JOYRIDE
    ch1.to_ac <= sfx_sample & "00" when sfx_busy_sig = '1' else ch1.incoming_sample & "00";
    ch2.to_ac <= sfx_sample & "00" when sfx_busy_sig = '1' else ch2.incoming_sample & "00";
